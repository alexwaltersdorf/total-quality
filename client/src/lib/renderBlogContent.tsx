 ...
 * titulos, "**negrito**" para enfase, linhas "- item" para listas e tabelas
 * "| col | col |". Antes disso, cada string do array virava um <p> literal —
 * o visitante via os caracteres ##, ** e | crus na pagina.
 *
 ...
 * de link (ex: "**hemograma completo**") sai em negrito E como link.
 */
...
function renderInline(text: string, currentPath: string, usedHrefs: Set<string>, keyPrefix: string): ReactNode[] {
  ...
  parts.forEach((part, i) => {
    ...
    const spans = linkifyText(raw, currentPath, usedHrefs).map((span, j) =>
      span.href ? (
        <Link key={`${keyPrefix}-${i}-${j}`} href={span.href} className="text-brand underline underline-offset-2 hover:opacity-80">
          {span.text}
        </Link>
      ) : (
        <span key={`${keyPrefix}-${i}-${j}`}>{span.text}</span>
      )
    );
    nodes.push(isBold ? <strong key={`${keyPrefix}-${i}`}>{spans}</strong> : <span key={`${keyPrefix}-${i}`}>{spans}</span>);
  });
